...
export const NavbarItem = forwardRef(function NavbarItem(
  {
    ...
    href?: string; // Explicitly define href to determine Link vs. Button
  ...
  const classes = clsx(
    "relative flex min-w-0 items-center gap-3 rounded-lg p-2 text-left text-base/6 font-medium text-zinc-950 sm:text-sm/5",
    "dark:text-white",
    current && "data-[hover]:bg-zinc-950/5 dark:data-[hover]:bg-white/5",
    className
  );
  ...
  return (
    <span className={clsx(className, "relative")}>
      ...
      {href ? (
        <Link
          {...(props as React.ComponentPropsWithoutRef<"a">)} // Cast to Link props
          href={href}
          to={href}
          className={classes}
          data-current={current ? "true" : undefined}
        ...
        <Headless.Button
          {...(props as React.ComponentPropsWithoutRef<"button">)} // Cast to Button props
          className={clsx("cursor-default", classes)}
          data-current={current ? "true" : undefined}
